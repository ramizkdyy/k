...
const AllNearbyPropertiesScreen = ({ navigation, route }) => {
    ...
    const getFilteredAndSortedProperties = () => {
        ...
        if (searchQuery.trim()) {
            ...
            filteredProperties = filteredProperties.filter(
                (property) =>
                    (property.ilanBasligi && property.ilanBasligi.toLowerCase().includes(query)) ||
                    (property.il && property.il.toLowerCase().includes(query)) ||
                    (property.ilce && property.ilce.toLowerCase().includes(query)) ||
                    (property.postDescription && property.postDescription.toLowerCase().includes(query))
            );
        }
    ...
    const renderPropertyItem = ({ item }) => (
        <TouchableOpacity
            className="bg-white rounded-2xl overflow-hidden mb-4 mx-4 shadow-lg border border-gray-100"
            ...
            <View className="p-4">
                ...
                <View className="flex-row items-center mb-3">
                    ...
                    <Text className="text-sm text-gray-600 ml-1">
                        {item.ilce && item.il ? `${item.ilce}, ${item.il}` : item.il || "Konum belirtilmemiş"}
                    </Text>
                </View>
            ...
            <View className="px-4 pt-4 pb-2">
                ...
                {userLocation && (
                    <Text className="text-sm text-gray-500">
                        {filteredProperties.length} ilan bulundu • Konum: {userLocation.latitude.toFixed(4)}, {userLocation.longitude.toFixed(4)}
                    </Text>
                )}
            </View>
            ...
            <SafeAreaView className="flex-1 bg-white">
                <View className="flex-1 justify-center items-center">
                    ...
                    <Text className="mt-3 text-base text-gray-500">
                        {locationLoading ? "Konum alınıyor..." : "Yakındaki evler yükleniyor..."}
                    </Text>
                </View>
            </SafeAreaView>
        );
